    ...
    def load(self, model_name="", bias_count=-1, input_count=-1, hidden_count=-1, output_count=-1, layer_count=-1, activation_values=[], min_diff=-1, learning_rate=-1, cycles=-1, hidden_shaped=False, normaliser_depth=-1, softmax=False):
        ...
        if not os.path.exists("./" + self.model_name):
            ...
            if layer_count == -1:
                ...
                self.activation_values = [4 for i in range(6)]+[6]
    ...
    def genetic_train(self, Data, deviation_coefficient, loop_count, pool_size):
        for i in range(loop_count):
            ...
            for weights_values in weights_values_list:
                weights_values_seq = c_double*len(weights_values)
...
class Data:
    ...
    def load(self, input_values_train, target_values_train, input_values_validate, target_values_validate, input_values_test, target_values_test):
        ...
        c_input_values_train_seq = c_double*len(input_values_train)
        c_target_values_train_seq = c_double*len(target_values_train)
        c_input_values_validate_seq = c_double*len(input_values_validate)
        c_target_values_validate_seq = c_double*len(target_values_validate)
        c_input_values_test_seq = c_double*len(input_values_test)
        c_target_values_test_seq = c_double*len(target_values_test)
