...
line_count = len(moving_average_close)-input_size-output_size

for i in range(line_count):
    print(i)
    model_num = floor(i/(line_count/model_count))
    
    to_write = ",".join(moving_average_close[i:i+input_size]) + ":" + ",".join(moving_average_close[i+input_size:i+input_size+output_size]) + "\n"

    if i%2 == 0:
        filew = open(ticker + "ABSOLUTE" + str(model_num) + "TRAIN.txt", "a")
    else:
        filew = open(ticker + "ABSOLUTE" + str(model_num) + "VALIDATE.txt", "a")
    filew.write(to_write)
    filew.close()
    
    filew = open(ticker + "ABSOLUTE" + str(model_num) + "TEST.txt", "a")
    filew.write(to_write)
    filew.close()