...
for i in range(line_count):
    ...
    if i%2 == 0:
        filew = open(ticker + "RATIOTWITTER" + str(model_num) + "TRAIN.txt", "a")
    else:
        filew = open(ticker + "RATIOTWITTER" + str(model_num) + "VALIDATE.txt", "a")
    filew.write(to_write)
    filew.close()
    
    filew = open(ticker + "RATIOTWITTER" + str(model_num) + "TEST.txt", "a")
    filew.write(to_write)
    filew.close()